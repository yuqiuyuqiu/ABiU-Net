...
def gradient_central_diff(input, cuda):#Qiuyu
    return input, input
    kernel = [[1, 0, -1]]
...
def numerical_gradients_2d(input, cuda=False):
    ...
    it matches numpy.gradient(image) with the difference than here output=x,y for an image while there output=y,x
    :param input: N, C, H, W
...
def convTri(input, r, cuda=False):#Qiuyu
    """
